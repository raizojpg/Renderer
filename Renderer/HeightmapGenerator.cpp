...
struct BMPHeader
{
	...
	std::uint32_t biSizeImagePerlin;
...
public:
	...
	ImagePerlin(std::size_t width, std::size_t height)
		: m_data(width* height)
...
int main()
{
	...
	ImagePerlin ImagePerlin{ 512, 512 };
	...
	for (;;)
	{
		...
		const double fx = (frequency / ImagePerlin.width());
		const double fy = (frequency / ImagePerlin.height());

		for (std::int32_t y = 0; y < ImagePerlin.height(); ++y)
		{
			for (std::int32_t x = 0; x < ImagePerlin.width(); ++x)
			{
				const RGB color(perlin.octave2D_01((x * fx), (y * fy), octaves));
				ImagePerlin.set(x, y, color);
			}
		}

		std::stringstream ss;
		ss <<"resources/"<< 'f' << frequency << 'o' << octaves << '_' << seed << ".bmp";

		if (ImagePerlin.saveBMP(ss.str()))
		{
